...
* O padrão sugere a separação do código que cria e o "código que usa" (código cliente) o objeto
* Trata da criação de objetos complexos (complexos de verdade), como:
  - Objetos com construtores muito complexos
  - Objetos compostos de vários objetos (padrão de projeto Composite)
...
- Ajuda na aplicação dos princípios SRP ("S" do padrão SOLID) e OCP ("O" do padrão SOLID)
